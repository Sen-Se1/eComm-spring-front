...
const WhichlistPage = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const response = await axios.get(`${process.env.API_HOST}/Commands/user/${id}`);
  ...
  const handleDelete = async (idArt) => {
    ...
    try {
      const response = await axios.delete(`${process.env.API_HOST}/Commands/deleteFC`, {data: data});
      if (response.status === 200) {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      const response = await axios.delete(`${process.env.API_HOST}/Commands/deleteAllProduct`, {data: data});
      if (response.status === 200) {
        setWishlist([]);
      }
    } catch (error) {
